...
This document provides practical examples of how to use the flutter_engine_cli CLI tool.
...
flutter_engine_cli add --name auth --state riverpod --with-g-routes
flutter_engine_cli add --name products --state bloc --with-g-routes
flutter_engine_cli add --name cart --state riverpod --with-g-routes
flutter_engine_cli add --name profile
```
...
After running `flutter_engine_cli init`, you can add additional dependencies:
...
dependencies:
  flutter:
    sdk: flutter
  # Generated by flutter_engine_cli
  get_it: ^7.6.0
  dartz: ^0.10.1
  equatable: ^2.0.5
...
After using flutter_engine_cli, your project is ready for:
